...
class Werewolf(Role):
    ...
    def do_action(self, response_prompt, phase_prompt, game_state):
        ...
        if len(self.werewolf_players) == 2:
            ...
            # 获取狼人1和狼人2的初始消息列表
            self._add_message(
                'werewolf1', werewolf_1.client.messages.copy()[0])
            self._add_message(
                'werewolf2', werewolf_2.client.messages.copy()[0])
            ...
            self._add_message(
                'werewolf1', {"role": "assistant", "content": werewolf_1_response})
            ...
            while not consensus and rounds < max_rounds:
                ...
                self._add_message(
                    'werewolf2', {"role": "assistant", "content": werewolf_2_response})
            ...
            werewolf_night_prompt = GameRulePrompt().get_night_action_prompt(
                role='werewolf', day_count=game_state.get_day_count(), player_id=werewolf.player_id)
            self._add_message(player, {
            ...
            self._add_message(
                player, {"role": "assistant", "content": werewolf_response})
